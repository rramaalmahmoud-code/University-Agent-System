    ...
    public class ArController : Controller
    {
      ...
      new
      {
        ...
        public IActionResult AddStudent(StudentViewModel model)
        {
            ...
            ModelState.Remove("studentNameEnglish");
            ...
            else
            {
                ...
                if (selectedMajor == null)
                {
                    ModelState.AddModelError(
                        nameof(StudentViewModel.AdmissionMajorId),
                        "التخصص المختار غير متاح للقبول.");
                }
                         ...
                         model.degreeId != selectedMajor.DegreeCode)
                {
                    ModelState.AddModelError(
                        nameof(StudentViewModel.AdmissionMajorId),
                        "التخصص المختار لا يتبع الكلية أو الدرجة المحددة.");
                }
                else
                {
            ...
            try
            {
                ...
                string agentName = GetAgentFullName(model.agentId) ?? "وكيلك";
                ...
                ViewBag.SuccessMessage = "تمت إضافة الطالب بنجاح!";
            }
            catch (Exception ex)
            {
        ...
        public string GetAgentFullName(int? agentId)
        {
            ...
            using (IDbConnection db = new SqlConnection(connectionString))
            {
                return db.QueryFirstOrDefault<string>(
                    "SELECT agentNameArabic FROM Agents WHERE agentId = @AgentId",
                    new { AgentId = agentId });
            }
        }
...
<td style=""color:black;font-family:Lato,Helvetica,Arial,sans-serif;padding:40px 30px 0 30px;line-height:25px;"" bgcolor=""white"" align=""left"">
 <p>عزيزي  {studentName},</p>
            <p>تم إرسال تسجيلك من قبل وكيلك <strong>{agentName}</strong>.</p>
...
<p><a href='{approvalUrl}'>اضغط هنا للموافقة</a></p> </td></tr></tbody></table></td></tr></tbody></table></div></div></div>
        ...
        private void ValidateUniqueFields(StudentViewModel model)
        {
            ...
            if (emailExists)
            {
                ModelState.AddModelError("studentEmail", "هذا البريد الإلكتروني مسجل بالفعل.");
            }

            string userType = GetUserType();
            if (userType == "Admin" && !model.agentId.HasValue)
            {
                ModelState.AddModelError("agentId", "يجب تعيين وكيل .");
            }
        ...
        private void ValidateUniqueFields(StudentViewModel model, student existingStudent)
        {
            ...
            if (!string.Equals(model.studentEmail, existingStudent.studentEmail, StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (emailExists)
                {
                    ModelState.AddModelError("studentEmail", "هذا البريد الإلكتروني مستخدم بالفعل.");
                }
            }
            ...
            if (!string.Equals(model.nationalId, existingStudent.nationalId, StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (idExists)
                {
                    ModelState.AddModelError("nationalId", "الرقم الوطني هذا مستخدم بالفعل.");
                }
            }
        }
    ...
    : (model.studentId > 0 ? model.studentId : (int?)null);
            ...
            {
                var agentSql = @"SELECT agentNameArabic 
                     FROM Agents 
            ...
            //ViewBag.SuccessMessage = "تم تحديث معلومات الطالب بنجاح!";
            ////PopulateDropdowns(model);
            ////return View("EditStudent", model);
...
";
        ...
        public IActionResult Login(LoginViewModel model)
        {
            ...
            if (user.active != 1)
            {
                ViewBag.ErrorMessage = "حسابك غير نشط حالياً بسبب التجميد أو الحظر";
                return View(model);
            }
            ...
            if (user.UserType.userTypeId == 3)
            {
                ...
                if (agentExists == null)
                {
                    ViewBag.ErrorMessage = "لم يتم العثور على حساب الوكيل. يرجى التواصل مع الدعم.";
                    return View(model);
                }
            }
        ...
        [Route("Ar/StudentInfo")]
        //[Route("Ar/StudentInfo/{id}")]
...
s.active,
 ...
 c.nationalityArabic AS Country,
  ...
  LEFT JOIN Statuses st ON s.statusId = st.statusId
      WHERE studentId = @studentId",
            ...
            if (student != null)
            {
                ...
                student.isActive = student.active == 1 ? "نشط" : "غير نشط";
            }
      ...
      @"SELECT Major_Name_S ,Major_Name
      FROM major_info1_vw 
      ...
      new { major_no = studentInfo.major_no }
        ...
        @"SELECT DISTINCT Faculty_Name_S ,Faculty_Name
      FROM major_info1_vw 
      WHERE Faculty_no = :Faculty_no ",
        new { Faculty_no = studentInfo.Faculty_no }
            ...
            return View("~/Views/Ar/StudentInfo.cshtml", student);
        }
